...
#reducing quality in 81 doesnt work yet.
...
Write-Host 'DEMUX...'
RunProcess "$VideoToolsDirectory\python-3.7.6.amd64\python.exe" """$VideoToolsDirectory\src\app.py"" -demux -if ""$DirectoryName\BL_EL_RPU.hevc"" -bl_out ""$DirectoryName\BL.hevc"" -el_out ""$DirectoryName\EL_RPU.hevc"""
...
Write-Host 'Mux and convert to DV8.1 to create BL_EL_RPU.hevc...'
RunProcess "$VideoToolsDirectory\python-3.7.6.amd64\python.exe" " ""$VideoToolsDirectory\src\app.py"" -mux -mode 2 -skip_hdr10plus -bl ""$DirectoryName\BL2.hevc"" -el ""$DirectoryName\EL_RPU.hevc"" -of ""$DirectoryName\BL_EL_RPU2.hevc"""


Write-Host 'Creating MP4...'
RunProcess "$VideoToolsDirectory\mp4muxer.exe" " -i ""$DirectoryName\BL_EL_RPU2.hevc"" -i ""$DirectoryName\audio.ac3"" --media-lang eng --dv-profile 8 --dv-bl-compatible-id 1 -o ""$FinalOutputDirectory\$FileName.mp4""  --overwrite"
